<script type="text/html" data-template-name="tcp in TSDV">
    <div class="form-row">
        <label for="node-input-server"><i class="fa fa-dot-circle-o"></i> <span data-i18n="tcpin.label.type"></span>Type</label>
        <select id="node-input-server" style="width:120px; margin-right:5px;">
            <option value="server" data-i18n="tcpin.type.listen">Listen on</option>
            <option value="client" data-i18n="tcpin.type.connect">Connect to</option>
        </select>
        <span data-i18n="tcpin.label.port">port</span> <input type="text" id="node-input-port" style="width:65px">
    </div>
    <div class="form-row hidden" id="node-input-host-row" style="padding-left: 110px;">
        <span data-i18n="tcpin.label.host">at host</span> <input type="text" id="node-input-host" placeholder="localhost" style="width: 60%;">
    </div>

    <div class="form-row">
        <label><i class="fa fa-sign-out"></i> <span data-i18n="tcpin.label.output">Output</span></label>
        <select id="node-input-datamode" style="width:110px;">
            <option value="stream" data-i18n="tcpin.output.stream">stream</option>
            <option value="single" data-i18n="tcpin.output.single">single</option>
        </select>
        <select id="node-input-datatype" style="width:140px;">
            <option value="buffer" data-i18n="tcpin.output.buffer">buffer</option>
            <option value="utf8" data-i18n="tcpin.output.string"string</option>
            <option value="base64" data-i18n="tcpin.output.base64">base64</option>
        </select>
    ...
    <div class="form-row">
        <label for="node-input-topic">topic<i class="fa fa-tasks"></i> <span data-i18n="common.label.topic"></span></label>
...
<script type="text/javascript">
    RED.nodes.registerType('tcp in TSDV',{
        category: 'network',
        color:"Pink",
        defaults: {
            name: {value:""},
        ...
        label: function() {
            return this.name || "tcp-TSDV:"+(this.host?this.host+":":"")+this.port;
        },
        labelStyle: function() {
...
<script type="text/html" data-template-name="tcp out TSDV">
    <div class="form-row">
...
<script type="text/javascript">
    RED.nodes.registerType('tcp out TSDV',{
        category: 'network',
        color:"#80bfff",
        defaults: {
...
<script type="text/html" data-template-name="tcp request TSDV">
    <div class="form-row">
        <label for="node-input-server" placeholder="host"><i class="fa fa-globe"></i> <span data-i18n="tcpin.label.server"></span>Server</label>
        <input type="text" id="node-input-server" placeholder="ip.address" style="width:45%">
        <span data-i18n="tcpin.label.port" placeholder="port"></span>
        <input type="text" id="node-input-port" style="width:60px">
    </div>
    <div class="form-row">
        <label for="node-input-out"><i class="fa fa-sign-out"></i> <span data-i18n="tcpin.label.return"></span>Return</label>
        <select type="text" id="node-input-out" style="width:54%;">
            <option value="time" data-i18n="tcpin.return.timeout">after</option>
            <option value="char" data-i18n="tcpin.return.character">when char recieved is</option>
            <option value="count" data-i18n="tcpin.return.number">a fixed number of char</option>
            <option value="sit" data-i18n="tcpin.return.never">never</option>
            <option value="immed" data-i18n="tcpin.return.immed">immediately</option>
        </select>
...
<script type="text/javascript">
    RED.nodes.registerType('tcp request TSDV',{
        category: 'network',
        color:"#80bfff",
        defaults: {
            server: {value:""},
        ...
        icon: "bridge-dash.svg",
        label: function() {
            return this.name || "tcp-TSDV:"+(this.server?this.server+":":"")+this.port;
        },
        labelStyle: function() {
            return this.name?"node_label_italic":"";
        },
